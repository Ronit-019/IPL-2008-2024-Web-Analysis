...
deli = pd.read_csv('deliveries.xls')
...
category = st.sidebar.selectbox("Select a Major Category",
                        ["About the Project","Team Analysis", "Player Analysis", "Batting Performance",
                         "Bowling Performance","Head-to-Head Analysis"])
...
if category == "Team Analysis":
    ...
    if sub_option == "Team Tally":
        ...
        y1 = helper.season_list(match)
        year = st.selectbox("Select Season", y1)
        ...
        y2 = helper.season_list(match)
        year = st.sidebar.selectbox("Select Season", y2)
    ...
    if sub_option == "Top 10 Batsman":
        ...
        y3 = helper.season_list(match)
        year = st.selectbox("Select Season", y3)
        df = Player_Analysis.top_10_batter(year)  # Fetch Top Batsmen data
        st.dataframe(df)

    elif sub_option == "Top 10 Bowler":
        st.header('Top 10 Bowler')
        y4 = helper.season_list(match)
        year = st.selectbox("Select Season", y4)
        ...
        y11 = helper.season_list(match)
        year = st.selectbox("Select Season", y11)
    ...
    if sub_option == "Strike Rate And Average":
        ...
        y12 = helper.season_list(match)
        year = st.selectbox("Select Season", y12)
    ...
    if sub_option == "Bowler Economy Rate And Strike Rate":
       ...
       st.dataframe(df)
        ...
        y13 = helper.season_list(match)
        year = st.selectbox("Select Season", y13)
